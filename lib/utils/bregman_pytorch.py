...
def sinkhorn(a, b, C, reg=1e-1, method='sinkhorn', maxIter=1000, tau=1e3,
             stopThr=1e-9, verbose=False, log=True, warm_start=None, eval_freq=10, print_freq=200, world_size=None, **kwargs):
    """
    ...
    elif method.lower() == 'distributed_sinkhorn':
        return distributed_sinkhorn(C, epsilon=0.05, sinkhorn_iterations=3, world_size=world_size)
    else:
...
def distributed_sinkhorn(out, epsilon=0.05, sinkhorn_iterations=3, world_size=None):
    """
    out: [batch_size, num_prototypes]
    epsilon: temperature (same as args.epsilon)
    sinkhorn_iterations: number of iterations (same as args.sinkhorn_iterations)
    world_size: number of GPUs (DDP world size)
    """

    Q = torch.exp(out / epsilon).t()  # [K, B]
    K, B = Q.shape

    # get world size automatically if not passed
    if world_size is None:
        world_size = dist.get_world_size() if dist.is_initialized() else 1

    # total number of samples across GPUs
    B_total = B * world_size

    # normalize so Q sums to 1 over all GPUs
    sum_Q = torch.sum(Q)
    ...
    for _ in range(sinkhorn_iterations):
        # normalize rows: each prototype should get 1/K of total mass
        ...
        # normalize columns: each sample gets 1/B_total mass
        sum_of_cols = torch.sum(Q, dim=0, keepdim=True)
        Q /= (sum_of_cols + 1e-12)
        Q /= B_total

    Q *= B_total  # make columns sum to 1
