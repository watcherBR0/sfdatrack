...
class Got10k_snowy(BaseVideoDataset):
    """ Got10k_snowy dataset.
    ...
    def _read_bb_anno(self, seq_path, pl_path):
        ...
        #0712
        if pl_path != '':
            bb_anno_haze_file = os.path.join(pl_path, seq_path.split('/')[-1], 'pl.txt')
            if not os.path.exists(bb_anno_haze_file):
                return torch.tensor(gt)
                # raise FileNotFoundError(f"File not found: {bb_anno_haze_file}")
            gt_merge = merge_txt_files(bb_anno_file, bb_anno_haze_file)
