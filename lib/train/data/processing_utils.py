...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    #0709
    #valid only crop change
    # def map_box_back(gt, pred_box: list, resize_factor: float):
    #     cx_prev, cy_prev = gt[0] + 0.5 * gt[2], gt[1] + 0.5 * gt[3]
    #     cx, cy, w, h = pred_box
    #     half_side = 0.5 * 128 / resize_factor
    ...
    # H, W = 720, 1280
    # box_crop_back = clip_box(map_box_back(box_gt[0], box_xywh_to_xyxy(box_crop[0]), resize_factors[0]), H, W, margin=0)

    return frames_crop, box_crop, att_mask, masks_crop, box_extract, resize_factors
