...
def normalize_hpp_prototype(model):
    """Keep HPP prototypes on the hypersphere without changing their shape."""
    with torch.no_grad():
        prototype_weight = F.normalize(model.prototype.weight, dim=1, p=2)
        model.prototype.weight.copy_(prototype_weight)


def update_ema_teacher(student_model, teacher_model, keep_rate):
    """Update the EMA teacher from the student model without direct prototype sync."""
    # Student prototypes are normalized after optimizer.step(); teacher prototypes are normalized after EMA.
    normalize_hpp_prototype(student_model)
    ...
    with torch.no_grad():
        ...
        for key, value in teacher_model.state_dict().items():
            if key in student_state_dict.keys():
    ...
    normalize_hpp_prototype(teacher_model)
...
class BaseTrainer:
    ...
    def update_net_extreme_params(self, keep_rate=0.9996):
        student_model = self.actor.net.module if multigpu.is_multi_gpu(self.actor.net) else self.actor.net
        teacher_model = self.actor.net_extreme.module if multigpu.is_multi_gpu(
            self.actor.net_extreme) else self.actor.net_extreme

        update_ema_teacher(student_model, teacher_model, keep_rate)


    def move_pgn_to_teacher(self):
        student_net = self.actor.net.module if multigpu.is_multi_gpu(self.actor.net) else self.actor.net
        teacher_net = self.actor.net_extreme.module if multigpu.is_multi_gpu(
            self.actor.net_extreme) else self.actor.net_extreme
        ...
        for key, value in teacher_net.state_dict().items():
            if key in student_state_dict.keys():
                if 'pgn_module' in key:
                    teacher_state_dict[key] = student_state_dict[key]
                else:
        ...
        for i in range(num_tries):
            ...
            for epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                if save_every_epoch or (epoch % 20 == 0 and epoch >= 0) or epoch in save_epochs \
                        or (epoch % 10 == 0 and epoch >= 50) or (epoch % 3 == 0 and epoch >= 30 and epoch <= 60):

                # if epoch in save_epochs or (epoch >= 50 and epoch % 3 == 0):
                    if self._checkpoint_dir:
                        if self.settings.local_rank in [-1, 0]:
                            self.save_checkpoint()
                            self.save_net_extreme_checkpoint()
    ...
    def save_net_extreme_checkpoint(self, eval=False):
        """Saves a checkpoint of the network and other variables."""
    ...
    def load_checkpoint(self, checkpoint=None, fields=None, ignore_fields=None, load_constructor=False):
        ...
        if checkpoint is None:
            ...
            checkpoint_path = '/home/wzq/SDATrack/output/checkpoints/train/sfdatrack/baseline_256/sfdatrack_ep0180.pth.tar'
            checkpoint_path_extreme = '/home/wzq/SDATrack/output/checkpoints/train/sfdatrack/baseline_256/sfdatrack_extreme_ep0180.pth.tar'
