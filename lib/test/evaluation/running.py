...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'got10k_train_haze', 'got10k_train_dark', "got10k_train_rainy"]:
    ...
    def save_bb(file, data, dataset=""):
        if dataset == 'got10k_train_haze' or dataset == 'got10k_train_dark' or dataset == "got10k_train_rainy":
    ...
    for key, data in output.items():
        ...
        #0715
        if key == 'target_bbox':
            if seq.dataset == 'got10k_train_haze' or seq.dataset == 'got10k_train_dark' or seq.dataset == "got10k_train_rainy":
                if isinstance(data[0], (dict, OrderedDict)):
                    ...
                    # Single-object mode
                    bbox_file = '{}/pl.txt'.format(base_results_path)
                    if not os.path.exists(bbox_file):
                        dir_path = os.path.dirname(bbox_file)
                        os.makedirs(dir_path, exist_ok=True)
                        open(bbox_file, 'a').close()
                    save_bb(bbox_file, data, dataset=seq.dataset)
            else:
                if isinstance(data[0], (dict, OrderedDict)):
                    data_dict = _convert_dict(data)
        ...
        if key == 'all_scores':
            ...
            if seq.dataset != 'got10k_train_haze' and seq.dataset != 'got10k_train_dark' and seq.dataset != "got10k_train_rainy":
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
            elif seq.dataset == 'got10k_train_haze' or seq.dataset == 'got10k_train_dark' or seq.dataset == "got10k_train_rainy":
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}/pl.txt'.format(base_results_path)
            else:
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8, pl_produce=False, save_dir_name=''):
    global VISDOM
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                tracker_name, tracker_param, dataset_name, ep_id, run_id = tracker_info
                tracker = Tracker(tracker_name, tracker_param, dataset_name, ep_id, run_id, debug=debug, pl_produce=pl_produce, save_dir_name=save_dir_name)
        ...
        param_list = [(seq, Tracker(*tracker_info, save_dir_name=save_dir_name), debug, num_gpus) for seq, tracker_info in product(dataset, trackers)]
