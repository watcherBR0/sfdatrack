...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    # scores = torch.tensor([54.58, 53.13, 53.11, 52.55, 51.65, 51.33, 45.96, 45.41])
    # scores = torch.tensor([62.21, 62.87, 62.48, 59.23, 62.84, 61.17, 66.07, 63.01])
    # scores = torch.tensor([81.12, 80.56, 80.57, 77.43,80.23 , 79.87, 85.72, 81.57])
    # scores = torch.tensor([75.45,   80.15,    78.22,   77.43,   79.32,   75.61,    86.05,   78.70])
    # scores = torch.tensor([64.03,   63.84,    63.57,  59.23,   63.95, 63.12,    66.75,   63.75])


    scores = torch.tensor([84.11,   83.32,    83.36,  77.43,   83.56, 83.55,    87.60,   83.28])
    # ev, ar, hip, os, od , aqa, umda, seq


    # SFDATrack, ARTrackV2, EVPTrack, decpt, seqtrack, aqatrack, SMAT, avtrack
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    if 'success' in plot_types:
        ...
        success_plot_opts = {'plot_type': 'success', 'legend_loc': 'lower left', 'xlabel': 'Overlap threshold',
                             'ylabel': 'Overlap Precision [%]', 'xlim': (0, 1.0), 'ylim': (0, 88), 'title': 'Success\ plots\ on\ DTB70\_rainy'}
    ...
    if 'prec' in plot_types:
        ...
        precision_plot_opts = {'plot_type': 'precision', 'legend_loc': 'lower right',
                               ...
                               'xlim': (0, 50), 'ylim': (0, 100), 'title': 'Precision\ plots\ on\ DTB70\_rainy'}
