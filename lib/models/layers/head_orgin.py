...
class CenterPredictor(nn.Module, ):
    ...
    def forward(self, x, feat_size, gt_score_map=None, loader_type='train_mix'):
        """ Forward pass with input x. """
        score_map_ctr, size_map, offset_map = self.get_score_map(x)

        # self.cnt += 1
        # save_feature_map(score_map_ctr, '/home/ysy/zr/lightUAV_pgn/heat_map', self.cnt)
        # print(self.cnt)

        # 0718
        # save_individual_heatmaps(score_map_ctr, save_dir="/nvme0n1/whj_file/models/Light-UAV-Track-Dual_0702/heatmap_test_sigmoid", type='sigmoid')

        # assert gt_score_map is None
        # if True:
        #     bbox = self.cal_bbox(score_map_ctr, feat_size, size_map, offset_map)
        #     bbox_extreme = self.cal_bbox_extreme(score_map_ctr, feat_size, size_map, offset_map)
        # else:
