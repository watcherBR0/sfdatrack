...
class CenterPredictor(nn.Module, ):
    ...
    def forward(self, x, feat_size, gt_score_map=None, loader_type='train_mix', positions=None):
        ...
        out = {'score_map': score_map_ctr, 'size_map': size_map, 'offset_map': offset_map}

        if loader_type == 'train_extreme':
            bbox, topk_score = self.cal_bbox_extreme(score_map_ctr, feat_size, size_map, offset_map)
            out.update({'bbox': bbox, 'topk_score': topk_score})
            return out

        bbox, t_score, pos_auto = self.cal_bbox(score_map_ctr, feat_size, size_map, offset_map)
        out['bbox'] = bbox
        
        if positions is None:
            out.update({'t_score': t_score, 'positions': pos_auto})
        else:
            out['s_score'] = self.get_s_score(score_map_ctr, positions)
