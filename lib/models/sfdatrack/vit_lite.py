...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=8,
                 ...
                 act_layer=None, weight_init='', stage=False):
        """
        Args:
    ...
    def forward_cross(self, z, x, attention, temporal_query, cls_tokens):
        ...
        for i, blk in enumerate(self.cross_blocks):
            if i in self.mamba_layers:
                x = blk(x, len_t=len_template + self.cls_token_len, attention=attention)
    ...
    def forward(self, z, x, temporal_query=None, **kwargs):
        ...
        z_feature = self.forward_self(z, type='z', attention=self.attention)
        ...
        if T_z > 1:  # multiple memory frames
            z_feature = z_feature.view(B, T_z, -1, z.size()[-1]).contiguous()
            z_feature = z_feature.flatten(1, 2).transpose(-1, -2)
        
        x_feature, aux_dict = self.forward_cross(z_feature, x_feature, self.attention, temporal_query, cls_tokens)

        return x_feature, aux_dict
...
def vit_lite_patch16_224(pretrained=False, **kwargs):
    """
