...
from timm.layers import Mlp, DropPath, to_2tuple, trunc_normal_, lecun_normal_
from timm.models import register_model

from lib.models.layers.patch_embed import PatchEmbed
# from lib.models.layers.tlpgn_topk import TLPGN
from lib.models.layers.tlpgn import TLPGN
from lib.models.layers.mamba_module import TokenSwapMamba, CrossMamba
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=8,
        ...
        self.swap_mamba_layers = nn.ModuleList(
            [TokenSwapMamba(self.embed_dim) for _ in range(2)]
        )


        self.deep_fusion_layers = nn.ModuleList(
            [CrossMamba(self.embed_dim) for _ in range(5)]
        )
    ...
    def forward_cross(self, z, x, attention):
        ...
        # swap_mamba1 = TokenSwapMamba(self.embed_dim)
        for block in self.swap_mamba_layers:
            input, input_copy, residual_x, residual_z = block(input, input_copy, residual_x, residual_z)


        for block in self.deep_fusion_layers:
            input, residual_x = block(input, residual_x, input_copy)

        return self.norm(input)


    def forward_self_prompt(self, tensor, prompt, type, attention, target_token=None):
        tensor = self.patch_embed(tensor)
        if type == 'x':
            tensor += self.pos_embed_x
        elif type == 'z':
    ...
    def forward(self, z, x=None, target_token=None, mode='train', loader_type='normal', epoch=0):
        if mode == 'z':
            ...
            # if epoch >= 251:
                # print("------------------------------------------")
                # print("Using Prompt Generation Networks!!!")
                # print("------------------------------------------")


            z_feature = self.forward_self(z, type='z', attention=self.attention, target_token=target_token)

            x_visual_context = self.pgn_module(x)
            x_feature = self.forward_self_prompt(x, x_visual_context, type='x', attention=self.attention)
        ...
        if 'npz' in pretrained:
            ...
            for key, value in state_dict.items():
                if key.startswith('backbone.'):
                    ...
                    if key.startswith('blocks.0') or key.startswith('blocks.1.') or key.startswith('blocks.2.') or key.startswith('blocks.3.'):
                        key=key.replace('blocks.', 'self_blocks.')
                        state_dict_new[key] = value

                    elif key.startswith('blocks.4'):
                        key = key.replace('blocks.4', 'cross_blocks.0')
                        state_dict_new[key] = value

                    elif key.startswith('blocks.5'):
                        key = key.replace('blocks.5', 'cross_blocks.1')
                        state_dict_new[key] = value

                    elif key.startswith('blocks.6'):
                        key = key.replace('blocks.6', 'cross_blocks.2')
                        state_dict_new[key] = value

                    elif key.startswith('blocks.7'):
                        key = key.replace('blocks.7', 'cross_blocks.3')
                        state_dict_new[key] = value
